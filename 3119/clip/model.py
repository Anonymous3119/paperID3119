...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,mask_matrix=None,trainable_noise=None):
        ...
        x_visual = x
        x = self.ln_post(x[:, 0, :])
        x_visual =self.ln_post(x_visual)
        clip_f = x
        if self.proj is not None:
            x = x @ self.proj
            x_visual=x_visual @ self.proj
        return x, x_visual,clip_f
