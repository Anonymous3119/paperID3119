...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, cam_label=None, return_attention = True):
...
class PatchEmbed_stride(nn.Module):
    ...
    def forward(self, x, mask_matrix, trainable_noise):
        B, C, H, W = x.shape
        ...
        if mask_matrix is not None:
            ...
            x = x * mask_matrix
            x = x.flatten(2).transpose(1, 2)
            zero_first_element_mask = (x[:, :, 0] == 0)
            x += zero_first_element_mask.unsqueeze(2) * trainable_noise
            x = x.type(stype)
        else:
            x = self.proj(x)
            x = x.flatten(2).transpose(1, 2)  ###32, 196, 768
...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, mask_matrix, trainable_noise):
        B = x.shape[0]
        if mask_matrix is not None:
            x = self.patch_embed(x,mask_matrix,trainable_noise)
        else:
            x = self.patch_embed(x,mask_matrix,trainable_noise)    ##假设是[b,c,h,w]输出是B，num，embed
    ...
    def forward(self, x, mask_matrix, trainable_noise):
        x, att = self.forward_features(x, mask_matrix, trainable_noise)
...
def vit_base_patch16_224_MAE(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, local_feature=False, aie_xishu=1.5, **kwargs):
    model = TransReID(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, drop_path_rate = drop_path_rate,\
        norm_layer=partial(nn.LayerNorm, eps=1e-6),  aie_xishu=aie_xishu, local_feature=local_feature, **kwargs)
